...
## Complete Mirror of Projects App Without Task Management

### Executive Summary
This implementation plan details the creation of a `video_production` Django app that **EXACTLY** mirrors the `projects` app structure, with the only differences being:
1. No task management functionality (no ProductTask, ProjectTask, TaskAssignment, ActiveTimer, TimeSession, DailyTimeTotal, TimerActionLog, DailyRoster, Holiday, MiscHours)
2. No product subcategory field (ProductSubcategory model not needed)
3. All model names prefixed with "Video" instead of using base names
4. VP_ prefix for HS_ID instead of letter-number pattern

### CRITICAL REQUIREMENTS
- **DO NOT** add any fields that aren't in the projects app
- **DO NOT** modify any existing projects app code
